...
/**
 * A4 purchase bill — rendered inside a hidden div, printed via window.print()
 */
const PurchaseBill = forwardRef<HTMLDivElement, { data: BillData }>(({ data }, ref) => {
  ...
  return (
    <div ref={ref} dir="rtl" className="bill-root bg-white text-slate-900" style={{ fontFamily: 'Vazirmatn, Tahoma, sans-serif' }}>
      ...
      <div className="text-center border-b-2 border-slate-800 pb-4 mb-4">
        <h1 className="text-2xl font-extrabold tracking-tight">بل خرید</h1>
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm mb-5 border border-slate-200 rounded-lg p-4 bg-slate-50">
        <div className="flex gap-2">
          ...
          <span className="font-bold">{data.invoice_number || `#${data.purchase_id}`}</span>
        </div>
        <div className="flex gap-2">
        ...
        <div className="flex gap-2">
          <span className="text-slate-500 w-24 shrink-0">تامین‌کننده:</span>
        ...
        {data.due_date && (
          <div className="flex gap-2">
            <span className="text-slate-500 w-24 shrink-0">سررسید:</span>
            <span className="font-bold">{new Date(data.due_date).toLocaleDateString('fa-AF')}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end mb-6">
        <div className="w-72 border border-slate-300 rounded-lg overflow-hidden">
          <div className="flex justify-between px-4 py-2.5 text-sm border-b border-slate-200">
            ...
            <span className="font-bold">{fmt(data.total_amount)} AFN</span>
          </div>
          <div className="flex justify-between px-4 py-2.5 text-sm border-b border-slate-200">
      ...
      {/* ─── Signatures ─── */}
      <div className="grid grid-cols-2 gap-8 mt-10 pt-4 border-t border-dashed border-slate-300">
        <div className="text-center">
          <div className="h-16" />
          <div className="border-t border-slate-400 pt-2">
            <p className="text-sm text-slate-600">امضاء خریدار</p>
          </div>
        </div>
        <div className="text-center">
          <div className="h-16" />
          <div className="border-t border-slate-400 pt-2">
            <p className="text-sm text-slate-600">امضاء تامین‌کننده</p>
          </div>
        </div>
      </div>

      {/* ─── Footer ─── */}
      <div className="text-center mt-8 pt-3 border-t border-slate-200">
...
PurchaseBill.displayName = 'PurchaseBill';
